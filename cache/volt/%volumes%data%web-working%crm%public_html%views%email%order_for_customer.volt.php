<?php $css_summary = 'background: #fff; padding: 15px;'; ?>
<?php $css_header = 'background: #4caf50; color: #fff; font-size: 20px; text-align: center; padding: 6px 12px;'; ?>
<?php $css_td_customer = 'padding: 7px; border: 1px solid #f0f0f0;'; ?>

<?php $css_subtotal = 'color: red; font-size: 14px; font-weight: bold'; ?>
<?php $css_total = 'color: red; font-size: 18px; font-weight: bold'; ?>
<?php $css_order_table = 'font-size: 14px; width: 100%'; ?>
<?php $css_image = 'width: 50px; height: 50px; object-fit: contain; float: left; margin-right: 10px;'; ?>

<?php $css_td_product = 'padding: 5px 10px; border: 1px solid #f0f0f0; vertical-align: top'; ?>
	...
	>
		...
		<div style="<?= $css_summary ?>">
			...
			<table style="<?= $css_order_table ?>" cellspacing="0" cellpadding="0">
				<?php foreach ($orderDetail->getItems('array') as $item) { ?>
				<tr>
					...
					<td style="white-space: nowrap; <?= $css_td_product ?>">
                        <?= $this->util->currencyFormat($item['price']) ?> x <?= $item['qty'] ?>
					</td>
					<td style="text-align: right; <?= $css_td_product ?>">
						<span style="<?= $css_subtotal ?>">
							<?= $this->util->currencyFormat($item['price'] * $item['qty']) ?>
						</span>
					</td>
				</tr>
				<?php } ?>

				<?php foreach ($orderDetail->getFee() as $fee) { ?>
					<tr>
						<td style="text-align: right; <?= $css_td_product ?>"  colspan="2" ><?= $fee['note'] ?></td>
						<td style="text-align: right; <?= $css_td_product ?>" ><b><?= $this->util->currencyFormat($fee['value']) ?></b></td>
					</tr>
				<?php } ?>
				...
				<tr>
					...
					<td style="text-align: right; <?= $css_td_product ?>; vertical-align:middle">
						<span style="<?= $css_total ?>"><?= $this->util->currencyFormat($orderDetail->total_price) ?></span>
					</td>
				</tr>
			</table>
